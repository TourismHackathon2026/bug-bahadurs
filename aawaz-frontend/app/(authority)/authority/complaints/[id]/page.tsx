...
export async function generateMetadata({
  params,
}: AuthorityComplaintDetailPageProps): Promise<Metadata> {
...
export default async function AuthorityComplaintDetailPage({
  params,
}: AuthorityComplaintDetailPageProps) {
  const { id } = await params;
  const [session, complaint] = await Promise.all([
    getSession(),
    getComplaintById(id),
  ]);
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            <h3 className="text-sm font-semibold">
              Evidence attached ({complaint.evidence.length})
            </h3>
                ...
                {complaint.evidence.map((file) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-2 truncate text-xs font-medium max-w-full px-2">
                        {name}
                      </p>
